  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 py-8 bg-bg-subtle">
      <div className="bg-bg border border-border shadow-md rounded-2xl p-8 max-w-xl w-full">
        ...
        {isRateLimited && (
          <RateLimitBanner message={rateLimitMsg} retryAfter="1 hour" />
        )}
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {errors.map((err, i) => (
            <p
              key={i}
              className="text-danger-text bg-danger-light border border-danger px-4 py-2 rounded-lg text-sm"
            >
              {err.msg}
            </p>
          ))}
  ...
  return (
    <div className="mt-1 p-4 bg-bg-subtle border border-border rounded-xl text-left flex flex-col gap-3">
      ...
      <p className="text-xs text-text-secondary">
        Enter the email you signed up with. We'll send a verification code, or
        redirect you directly if one was recently sent.
      </p>
